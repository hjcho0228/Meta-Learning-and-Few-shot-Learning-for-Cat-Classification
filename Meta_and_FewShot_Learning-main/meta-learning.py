...
def meta_test_HI(resnet, hyunbin_dir, IU_dir, transform, device):

    def compute_prototypes(support_set_dir):
        import glob
        image_paths = glob.glob(os.path.join(support_set_dir, "*.jpeg"))[:3]  # 첫 세 장을 Support Set으로 사용
        embeddings = []
        for img_path in image_paths:
            image = Image.open(img_path).convert("RGB")
            image = transform(image).unsqueeze(0).to(device)
    ...
    # 1) 카테고리별 프로토타입 계산
    hyunbin_prototype = compute_prototypes(hyunbin_dir)
    IU_prototype = compute_prototypes(IU_dir)
    prototypes = torch.stack([hyunbin_prototype, IU_prototype], dim=0)  # (2, embed_dim)
    print("Prototypes:", prototypes.sum(dim=1))

    # 2) Query 이미지 수집
    query_images = [(img_path, 0) for img_path in glob.glob(os.path.join(hyunbin_dir, "*.jpeg"))[3:]] + \
                   [(img_path, 1) for img_path in glob.glob(os.path.join(IU_dir, "*.jpeg"))[3:]]  # 첫 세 장 이후를 Query Set으로 사용
    
    # (선택) 예측 클래스 이름을 보기 좋게 표시하기 위한 매핑 (인덱스→이름)
    class_names = ["Hyunbin", "IU"]
    ...
    # 3) Query마다 예측
    for img_path, true_label in query_images:
        image = Image.open(img_path).convert("RGB")
        image = transform(image).unsqueeze(0).to(device)
        ...
        # 성능 기록
        total += 1
        correct += int(predicted_class_idx == true_label)
    ...
    if total > 0:
        ...
        print("No valid labels found for accuracy calculation.")
...
if __name__ == "__main__":
   ...
   # ResNet-50 모델 초기화 (저장된 가중치를 로드하기 위해)
    ...
    meta_test(resnet, cat1_dir, cat2_dir, transform)
    IU_dir = "./IU_images"
    hyunbin_dir = "./hyunbin_images"
    #meta_test_HI(resnet, hyunbin_dir, IU_dir, transform, device)
